...
func AfterMatchRoute(c *fst.Context) {
	if c.Req.Raw.Referer() != "https://www.tl50.com/" {
		c.AbortDirect(http.StatusHTTPVersionNotSupported, "只支持http/1.x协议")
		// c.SetRouteTo404()
		return
	} else if !strings.HasPrefix(c.Req.Raw.RemoteAddr, "10.10") {
		c.SetRouteTo404()
		return
	} else if c.Req.Raw.ProtoMajor == 2 {
		c.AbortDirect(http.StatusHTTPVersionNotSupported, "只支持http/1.x协议")
		return
	}
...
func UpdateBase(c *fst.Context) {
	...
	u := acc.SysUser{}
	ct := cf.DDemo.QueryPrimaryCache(&u, userId)
	cst.PanicIf(ct <= 0, cf.FaiNotFound)

	newName := c.GetStringMust("user_name")
	u.Name = newName
	if ct = cf.DDemo.UpdateColumns(&u, "name", "status"); ct <= 0 {
